...
<template>
  <v-sheet class="pa-1 mx-auto mx-md-6 mx-lg-15 mx-xl-auto xl-width my-9 my-lg-15">
    <v-card class="mx-3 mx-lg-12 my-3 my-lg-12 px-1 px-md-3 px-lg-9 py-3 py-lg-9" elevation="9" variant="tonal">
      ...
      <v-card class="px-3 px-lg-15 pb-3 pt-6 py-lg-15">
        <v-form ref="registrationForm" v-model="isValid" lazy-validation @submit.prevent="register">
          <v-row>
            <v-col>
              <v-text-field v-model="form.userName" :readonly="loading" :rules="userNameRules" class="mt-2" clearable
                label="User Name" />
            </v-col>
          </v-row>
          <v-row>
          ...
          <v-row>
            <v-col>
              <v-text-field v-model="passwordConfirm" :readonly="loading" :rules="[passwordMatch]"
                :append-icon="showPW ? 'fa:fas fa-duotone fa-eye' : 'fa:fas fa-duotone fa-eye-slash'"
                :type="showPW ? 'text' : 'password'" @click:append="$event => showPW = !showPW" counter clearable
          ...
          <v-row>
            <v-col>
              <v-btn :loading="loading" block class="reg-btn" color="success" size="large" type="submit" elevation="6"
                variant="tonal">
                Register
              </v-btn>
            </v-col>
